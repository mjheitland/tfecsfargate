...
To enable long arn format run:
aws ecs put-account-setting --name serviceLongArnFormat --value enabled --profile default
aws ecs put-account-setting --name taskLongArnFormat --value enabled --profile default
aws ecs put-account-setting --name containerInstanceLongArnFormat --value enabled --profile default

Check ECS version: https://aws.amazon.com/blogs/containers/aws-fargate-platform-versions-primer/ 
...
## Setup (takes about 10 min)
```
terraform init -backend-config=backend.config

terraform apply -var-file=dev.tfvars -auto-approve 

# use ALB's ARN to test that you can reach the container (runs on port 80, should return a web page)
```
...
| region | The AWS region to use for the dev environment's infrastructure`. | string | `eu-central-1` | no |
